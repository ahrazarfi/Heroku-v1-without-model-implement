...
def upload():
    ...
    if file and allowed_file(file.filename):
        ...
        newFile = Users(
            ...
            doctoruser=current_user.username,
        )
        ...
        if "img" in request.files:
            ...
            if type_ == "dia_ret":
                ...
                test_image = np.expand_dims(
                    test_image, axis=0
                )  # Add another dimension because the model was trained on (n,128,128,3)
                data = test_image

            elif type_ == "oct":
                test_image = imageio.imread(name)  # Read image using the PIL library
                test_image = resize_image_oct(
                    test_image
                )  # Resize the images to 128x128 pixels
                test_image = np.array(test_image)  # Convert the image to numpy array
                test_image = test_image / 255  # Scale the pixels between 0 and 1
            ...
            if type_ == "dia_ret":
                preds, pred_val = translate_retinopathy(
...
def resize_image_oct(image):
    resized_image = cv2.resize(
        image, (128, 128)
    )  # Resize all the images to 128X128 dimensions
    if len(resized_image.shape) != 3:
        resized_image = cv2.cvtColor(
            resized_image, cv2.COLOR_GRAY2RGB
        )  # Convert to RGB
