...
def disease_prediction_Handler(result):
    rem_list = [
        "empty",
        "type",
    ]  # source: "https://www.kite.com/python/answers/how-to-remove-multiple-keys-from-a-dictionary-in-python"
    if disease_URL == "dia":
        for key in rem_list:
            del result[key]
...
if st.button("Analyze"):
    if image is not None and disease_URL is not None:
        ...
        result = (
            res.json()
        )  # returns the response from the API in JSON format, returns a dictionary
